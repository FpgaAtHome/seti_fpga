...
std::vector<std::pair<float, float>> LoadTestData(std::string filename)
{
	...
	std::ifstream inDataFile(filename);
	std::string line;
...
TEST(LibFftTest2, Simple_ComplexToComplex_Fft_8_Wikipedia_Page_Data) {
	...
	fftwf_complex* in_data_tmp = ConvertVectorToArray(
							LoadTestData("./data/data_8.txt")
						);
	memcpy(in_data, in_data_tmp, fft_len*sizeof(fftwf_complex));
...
TEST(LibFftTest, LoadFile_16) {
	// GIVEN
	std::string filename("./data/data_16.txt");
...
TEST(LibFftTest, LoadFile_32) {
	// GIVEN
	std::string filename("./data/data_32.txt");
...
TEST(LibFftTest, Simple_ComplexToComplex_Fft_16_Simple_2) {
	...
	fftwf_plan analysis_plan = fftwf_plan_dft_1d(fft_len, in_data, out_data, FFTW_FORWARD, FFTW_MEASURE);
	fftwf_complex* in_data_tmp = ConvertVectorToArray(
							LoadTestData("./data/data_16_1.txt")
						);
	memcpy(in_data, in_data_tmp, fft_len*sizeof(fftwf_complex));
	...
	fftwf_complex* results = ConvertVectorToArray(
							LoadTestData("./data/data_16_1-results.txt")
						);
...
TEST(LibFftTest, UseLabViewFpga_ComplexToComplex_Fft_16_Simple_1) {
	...
	// WHEN
	int errorCode = 0;
	NiFpgaViExecutionMode fpgaViExecutionMode;
	errorCode = InitializeFpga(&fpgaViExecutionMode);
	std::cout << "\nEXE: " << fpgaViExecutionMode;

	cmplx64 complexDataIn[16];
	cmplx64 complexDataOut[16];
	for (int i=0; i<16; i++) {
		complexDataIn[i].re = in_data_tmp[i][0];
		complexDataIn[i].im = in_data_tmp[i][1];
	}
	errorCode = CalcData(complexDataIn, complexDataOut, 16, 16);
